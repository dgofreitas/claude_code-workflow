...
### Rule: Active Project Anchor — MANDATORY (multi-project umbrella)

In an umbrella install (one `.claude/`, several sub-project git repos), resolve the active project FIRST:
...
type: checkpoint
story: STORY-XXX
title: <story title from the STORY-XXX.md heading>
development: <story's Parent Epic id, or an explicit `development` label if the story has one>
epic: <Parent Epic id, if any>
status: in-progress
schema_version: 1
...
> **Frontmatter** (per `context/standards/artifact-frontmatter.md`): the checkpoint is the
> ONE mutable artifact, so it carries the live board fields. Source `title`/`development`/`epic`
> from the story: `title` = the STORY heading; `development` = the story's explicit `development`
> label if present, else the `Parent Epic` id; `epic` = the Parent Epic id (omit if none).
...
- Keep the frontmatter `status`/`updated` current as the story crosses gates (and set `coverage` at GATE 2) — see **Rule: Board Status Field**. This is the ONLY mutable frontmatter and drives the Bases story board.

### Rule: Board Status Field — MANDATORY

The checkpoint frontmatter `status` is the board's live column. TechLead updates it (and bumps `updated`) at each gate boundary — this is a frontmatter edit, NOT a checklist mark:

| Set `status:` to | When |
|------------------|------|
| `in-progress` | at checkpoint creation (implementation phase) |
| `in-qa` | GATE 2 passed (tests green) — QA pending/running |
| `in-review` | GATE 3 passed (QA `PASSED`) — review pending/running |
| `ready` | GATE 4 passed (review `APPROVED`) — MR pending / at DONE handoff |
| `blocked` | any gate BLOCKED or story blocked (also on `STORY-XXX-BLOCKED`) |

- At GATE 2, also set `coverage: <N>` in the frontmatter from test-engineer's reported coverage.
- `status: merged` is set by **merge-request-creator** when it creates the MR and marks `[x] Merge Request` — NOT by TechLead or Master (Master is router-only and never writes files).
- Never put `status` anywhere except the checkpoint frontmatter. Story/report frontmatter is immutable (identity only).
